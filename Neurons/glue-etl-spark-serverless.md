---
title: aws-glue-serverless-etl-multi-source-integration
created: 2025-08-15
modified: 2025-08-15
tags:
- service/aws-glue
...
- engine/apache-spark
aliases: ["AWS Glue", "serverless ETL", "데이터 통합", "apache spark"]
---
...
### 실전 적용
- 하이브리드 클라우드 데이터 통합에서 온프레미스 Oracle DB → S3 → Redshift 파이프라인 구축
- 멀티 소스 데이터 레이크 구축 시 다양한 JDBC 데이터베이스를 S3 중앙 저장소로 통합
- 데이터 웨어하우스 ETL에서 RDS/Aurora 운영 데이터를 Redshift 분석 환경으로 변환 및 적재
...
💡 추가 설명:
- **AWS Lambda** - 15분 실행 시간 제한과 메모리 제약으로 대용량 ETL에 부적합
- **Amazon EMR** - 수동 설정 필요하고 서버 관리가 필요해 요구사항에 맞지 않음
- **AWS Data Pipeline** - EC2 인스턴스 관리가 필요하고 자동 코드 생성 기능 없음
- **Amazon Kinesis Data Firehose** - 실시간 스트리밍 서비스로 배치 ETL 시나리오에 부적합